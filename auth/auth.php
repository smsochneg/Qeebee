...
class auth
{
    ...
    {
        $this->mysqli = connectDB();                                                    //connecting to db
        $users = $this->mysqli->query('SELECT * FROM users WHERE login="' . $login . '" OR email="'. $email .'"'); //looking if we have such user
    ...
    public function verify($code){                                                      //connect to db
        ...
        $tmp_db = $this->mysqli->query('SELECT * FROM tmp_user WHERE code="'.$code.'"');//looking for user with a code that user wrote
